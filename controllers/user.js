...
const getAllUsers = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, msg: "Failed to retrieve users" });
  }
};
...
const getUser = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, msg: "Failed to retrieve user" });
  }
};
...
const editUser = async (req, res) => {
  ...
  if (!authUtils.isUserOwner(sessionStr)) {
    return res
      .status(403)
      .json({ success: false, msg: "You are not the owner of the account" });
  }

  if (!id || (!username && !password && !email && !phone_number)) {
    return res.status(400).json({
      success: false,
      msg: "Please provide an id and at least one field to update",
    });
  }
  ...
  if (id) {
    try {
      ...
      if (result.affectedRows === 0) {
        res
          .status(400)
          .json({ success: false, msg: "User with id does not exist" });
      } else {
